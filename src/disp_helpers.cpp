...
int32_t arc_x(int32_t x, int32_t a, int32_t r) {
  float sxL = cos((a - 90) * DEG2RAD);
  return sxL * r + x;
}

int32_t arc_y(int32_t y, int32_t a, int32_t r) {
  float syL = sin((a - 90) * DEG2RAD);
  return syL * r + y;
}
...
void draw_arc_arrow(TFT_eSprite sprite, int32_t x, int32_t y, int32_t a, int32_t rx, int32_t ry, uint32_t color, int32_t w,
    ...
    if (inside) {
        int x1 = arc_x(x, a - w/2, rx + h);
        int y1 = arc_y(y, a - w/2, ry + h);
        int x2 = arc_x(x, a + w/2, rx + h);
        int y2 = arc_y(y, a + w/2, ry + h);
        int x3 = arc_x(x, a, rx);
        int y3 = arc_y(y, a, ry);
...
int fillArc(TFT_eSprite sprite, int x, int y, int start_angle, int seg_count, int rx, int ry, int w, unsigned int colour)
{
